...
void Employee::EmployeeSUpdate(int i, int j)
{
	...
	case Employee::Incinerator:
		...
		employeeS[i][j]->audio->Stop("neppa_.wav");//�R�₷
		employeeS[i][j]->audio->Stop("15347_.wav");//����
		employeeS[i][j]->audio->PlayWave("neppa_.wav",false);//�R�₷
		employeeS[i][j]->audio->PlayWave("15347_.wav", false);//����
