...
void ParticleManager2d::Update()
{
	...
	for (std::forward_list<Particle2d>::iterator it = particleList_.begin(); it != particleList_.end(); it++) {
		...
		//パーティクル更新
		it->particle_->SetPosition({ it->pos_.x, it->pos_.y, 0.0f });
		it->particle_->SetColor({ it->color_.x, it->color_.y, it->color_.z, it->alpha_ });
...
void ParticleManager2d::Add(int texNumber, int32_t life, XMFLOAT2 position, XMFLOAT2 velocity, XMFLOAT2 accel, XMFLOAT2 start_scale, XMFLOAT2 end_scale, XMFLOAT3 start_color, XMFLOAT3 end_color, float start_alpha, float end_alpha)
{
	// リストに要素を追加
	...
	p.particle_.reset(Sprite::Create(spriteCommon_, texNumber));
}