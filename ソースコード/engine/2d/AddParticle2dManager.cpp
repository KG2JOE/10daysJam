...
AddParticle2dManager* AddParticle2dManager::GetInstance()
{
	static AddParticle2dManager instance;
	return &instance;
}

void AddParticle2dManager::Initialize(ID3D12Device* device)
{
	// nullptrチェック
	assert(device);
...
void AddParticle2dManager::Update()
{
	HRESULT result;
...
void AddParticle2dManager::Draw(ID3D12GraphicsCommandList* cmdList)
{
...
void AddParticle2dManager::Add(int life, XMFLOAT3 position, XMFLOAT3 velocity, XMFLOAT3 accel, 
...
void AddParticle2dManager::InitializeDescriptorHeap()
{
...
void AddParticle2dManager::InitializeGraphicsPipeline()
{
	HRESULT result = S_FALSE;
...
void AddParticle2dManager::LoadTexture()
{
	HRESULT result = S_FALSE;
	...
	result = LoadFromWICFile(
		L"Resources/sprite/white1x1.png", WIC_FLAGS_NONE,
		&metadata, scratchImg);
	if (FAILED(result)) {
		assert(0);
	}
	...
	// テクスチャ用バッファの生成
	CD3DX12_HEAP_PROPERTIES texProperties = CD3DX12_HEAP_PROPERTIES(D3D12_CPU_PAGE_PROPERTY_WRITE_BACK, D3D12_MEMORY_POOL_L0);
	result = device->CreateCommittedResource(
		&texProperties,
		D3D12_HEAP_FLAG_NONE,
		&texresDesc,
...
void AddParticle2dManager::CreateModel()
{
	HRESULT result = S_FALSE;

	// 頂点バッファ生成
	CD3DX12_HEAP_PROPERTIES vertProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
	CD3DX12_RESOURCE_DESC vertDesc = CD3DX12_RESOURCE_DESC::Buffer(sizeof(VertexPos) * vertexCount);
	result = device->CreateCommittedResource(
		&vertProperties,
		D3D12_HEAP_FLAG_NONE,
		&vertDesc,
